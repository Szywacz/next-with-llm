 ...
 * ```
 */
export default function useOnOutsideClick(exceptionRefArr: React.RefObject<HTMLElement>[], callback: (event: MouseEvent | TouchEvent) => void) {
  useEffect(() => {
    function listener(event: MouseEvent | TouchEvent) {
      if (exceptionRefArr.some(exception => !exception.current || exception.current.contains(event.target as Node))) return;
